...
📍1️⃣Ваш Объект стажировки: {internship_object_name}
    ...
    try:
        ...
        if bot:
            ...
            if old_role_name == "Наставник" and len(active_mentorships) > 0:
                notification_text += f"\n\n⚠️ ВНИМАНИЕ: Ваши {len(active_mentorships)} стажеров остались без наставника. Обратитесь к рекрутеру для назначения нового наставника."
                ...
                notification_text += "\n\n✅ Ваши данные стажера (траектории, наставник) деактивированы."
    ...
    try:
        ...
        async with async_session() as session:
            ...
            if duplicates:
                ...
                logger.info("💡 Для очистки используйте команду администратора или редактор пользователя")
            else:
...
async def send_learning_path_assigned_notification(session: AsyncSession, trainee_id: int, learning_path_id: int, bot=None) -> None:
    ...
    try:
        ...
        message = (
            "🗺️<b>Вам доступна траектория обучения!</b>\n"
...
async def send_stage_completion_notification_to_trainee(session: AsyncSession, trainee_id: int, stage_id: int, bot=None) -> None:
    ...
    try:
        ...
        # Уведомление стажеру согласно ТЗ
        message = f"""🏆<b>Ваш прогресс</b>
{trajectory_progress}
✅ <b>Вы завершили {stage.name}!</b>
